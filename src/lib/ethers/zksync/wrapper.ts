...
import { types, Provider, Wallet } from 'zksync-ethers'
...
class WalletWrapper {
  ...
  async composeTransaction (
    ...
    let tx: types.TransactionRequest = {
      from: params.from, // todo: ? utils.getA.getAddress(params.from) : undefined,
      to: params.to,
      value: params.value,
      data: params.data,
      nonce: params.nonce
    ...
    // no tx.gasPrice on EIP-1559 transactions...
    if (tx?.gasPrice) {
      delete tx.gasPrice
    }
  ...
  async processEthCall (
    ...
    let tx: types.TransactionRequest = await this.composeTransaction(
      socket,
      params
    )
  ...
  async processTransaction (
    ...
    if (!tx?.to || tx.to === "") {
      const reason = `Sorry, no contract deploying transactions are currently supported.`
      throw {
        reason,
        body: {
          error: {
    ...
    if (!wallet) {
      const reason = `No private key available as to sign messages from '${params.from}'`
      throw {
        reason,
        body: {
          error: {
      ...
      // populate missing tx fields, if any
