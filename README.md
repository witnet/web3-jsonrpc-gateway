...
- `ETHERS_ESTIMATE_GAS_PRICE`: if set to `true`, the provider will be asked to estimate the gas price, before signing the transaction; if the provider-estimated gas price is greater than the one specified from client side (or `ETHERS_GAS_PRICE`, if none given), the transaction will be rejected by the gateway.
